...
// header for the exact command). Proves the Internet/eBadal/Reseller
...
// each of the three flags is genuinely independent (flipping one never
// affects the other two, in any of the 8 possible ON/OFF combinations), and
...
async function getPublicServices(): Promise<{ internetEnabled: boolean; ebadalEnabled: boolean; resellerEnabled: boolean }> {
...
test("default (no admin has ever touched this setting): every service is visible", async () => {
  ...
  assert.deepEqual(services, { internetEnabled: true, ebadalEnabled: true, resellerEnabled: true });
});
...
test("turning Internet off hides only Internet -- eBadal and Reseller stay visible", async () => {
  await setService("service_internet_enabled", false);
  assert.deepEqual(await getPublicServices(), { internetEnabled: false, ebadalEnabled: true, resellerEnabled: true });
});
...
test("turning Internet back on makes it visible again, with no effect on the others", async () => {
  ...
  assert.deepEqual(await getPublicServices(), { internetEnabled: true, ebadalEnabled: true, resellerEnabled: true });
});

test("turning eBadal off hides only eBadal", async () => {
  await setService("service_ebadal_enabled", false);
  assert.deepEqual(await getPublicServices(), { internetEnabled: true, ebadalEnabled: false, resellerEnabled: true });
});

test("turning Reseller off hides only Reseller -- it is not permanently hidden, and toggling it never touches Internet/eBadal", async () => {
  await setService("service_reseller_enabled", false);
  assert.deepEqual(await getPublicServices(), { internetEnabled: true, ebadalEnabled: true, resellerEnabled: false });
});
...
// All 8 possible ON/OFF combinations of the three independent flags,
...
const ALL_COMBINATIONS: Array<[boolean, boolean, boolean]> = [
  [true, true, true],
  [true, true, false],
  [true, false, true],
  [true, false, false],
...
for (const [internet, ebadal, reseller] of ALL_COMBINATIONS) {
  test(`combination internet=${internet} ebadal=${ebadal} reseller=${reseller} resolves exactly as set, independently`, async () => {
...
test("GET /admin/settings (staff) reflects the same three keys admins actually edit", async () => {
  await setService("service_internet_enabled", false);
...
test("an unknown settings key is rejected -- the three service keys must be spelled exactly as the backend expects", async () => {
