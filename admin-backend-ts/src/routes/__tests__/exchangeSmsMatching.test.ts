...
test("safe payout lifecycle: SMS match -> in_progress -> dial-attempt -> step1 -> step2 success -> Completed", async () => {
  ...
  assert.equal(start.step1UssdString, "*712*688000000*60#", "EVC Plus payout must dial *712*NUMBER*AMOUNT#, a whole-dollar amount with no spurious '*00', receiver normalized to the bare 9-digit local number");
...
test("payout USSD string uses the correct carrier code for each wallet: EVC Plus *712*, eDahab *110*", async () => {
  ...
  assert.equal(evcStart.step1UssdString, "*712*688000000*63#", "EVC Plus payout must dial *712*NUMBER*AMOUNT#, a whole-dollar amount with no spurious '*00'");
