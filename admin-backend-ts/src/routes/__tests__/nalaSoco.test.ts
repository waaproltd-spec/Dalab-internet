...
// only when an image actually exists, Admin CRUD (create with/without an
// image, update without touching the image, replace the image, explicitly
// remove the image, delete), and that a plain Admin (not just Super Admin)
// can manage posts since this uses requireStaff(), not promo_images'
// stricter Super-Admin-only gate.
...
test("POST /admin/nala-soco rejects a customer token (staff only)", async () => {
