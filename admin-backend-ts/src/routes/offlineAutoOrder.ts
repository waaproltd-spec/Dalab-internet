...
export type OfflineOrderPackagePickResult = { applied: boolean; reason: string | null };

/**
 * Sets offline_package_id on a customer's Offline Profile from the
 * Customer App's own SMS side-channel (see the Customer App's
 * OfflineOrderSms/MainActivity.kt and the Agent App's
 * OfflineOrderPickParser) -- a compact "DALAB-PKG:<packageId>" text the
 * customer's phone sends silently, over the SIM/signaling channel, the
 * instant "Iibso Hadda" is tapped, to the company's own payment number
 * (the same physical line the Agent's device holds and already monitors
 * for every other incoming SMS). This exists so matchOrCreateOfflineAutoOrder
 * above can find a complete profile (sender+destination+company+package)
 * WITHOUT depending on the customer's own phone ever reconnecting to the
 * internet -- PUT /customer/offline-profile (customers.routes.ts) is still
 * the profile's authoritative writer whenever the customer IS online (the
 * Customer Information screen, and the reconnect-triggered
 * syncPendingOfflineProfile retry this backs up, not replaces); this is
 * only ever a second, additional way the exact same field gets set.
 *
 * Deliberately mirrors matchOrCreateOfflineAutoOrder's own candidate
 * lookup (same normalizePhone + last-9-digit match, same "exactly one
 * profile or refuse" rule) rather than trusting a customerId from the SMS
 * body -- the SMS's only trustworthy identity signal is which phone number
 * it actually arrived from, same trust model the payment-matching SMS
 * pipeline already relies on everywhere else in this file. Idempotent: a
 * duplicate/retried upload of the same pick just re-applies the same
 * value, and this never creates or touches an order itself -- only
 * matchOrCreateOfflineAutoOrder (and its resweep retry) does that, so a
 * pick applied twice can never create a duplicate order.
 */
export async function applyOfflineOrderPackagePick(senderPhone: string, packageId: string): Promise<OfflineOrderPackagePickResult> {
  const target = normalizePhone(senderPhone);
  if (!target) return { applied: false, reason: null };

  const candidates = await query<{ id: string; offline_company_id: string }>(
    `SELECT id, offline_company_id FROM customers
     WHERE offline_sender_number IS NOT NULL AND offline_destination_number IS NOT NULL
       AND offline_company_id IS NOT NULL AND status='active'
       AND RIGHT(regexp_replace(offline_sender_number, '\\D', '', 'g'), 9) = $1`,
    [target]
  );
  if (candidates.length === 0) return { applied: false, reason: `No Offline Profile registered for phone ...${target}` };
  if (candidates.length > 1) {
    return { applied: false, reason: `${candidates.length} Offline Profiles are registered for phone ...${target} — ambiguous, refusing to apply` };
  }
  const profile = candidates[0];
  ...
  if (!pkg) return { applied: false, reason: "Package not found or inactive" };
  if (pkg.company_id !== profile.offline_company_id) {
    return { applied: false, reason: "Package does not belong to this profile's saved company" };
  }

  await query(`UPDATE customers SET offline_package_id=$1, offline_profile_updated_at=now() WHERE id=$2`, [pkg.id, profile.id]);
