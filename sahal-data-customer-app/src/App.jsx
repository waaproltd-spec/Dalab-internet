...
const TRANSLATIONS = {
  en: {
    ...
    // OTP
    otpTitle: "SAHAL DATA", otpSubtitle: "CODE", otpVerifyingNumber: "Xaqiijinta lambarka",
    ...
    // Settings page
    settingsTitle: "Setting", settingsTheme: "Theme", settingsThemeDesc: "Choose how SAHAL DATA looks",
    ...
    aboutBody: "SAHAL DATA lets you buy internet, calling, and SMS bundles from Hormuud, Somnet, Somtel, and Amtel in one app — with Macaash rewards on every purchase.",
    ...
    inviteFriendsBody: "Share SAHAL DATA with friends and family to get the better service experience ever.",
    inviteFriendsButton: "INVITE FRIENDS",
    // Rate
    rateTitle: "Rate SAHAL DATA", rateSubtitle: "How's your experience so far?", notNow: "Not now",
    // Delete account
  ...
  so: {
    ...
    // OTP
    otpTitle: "SAHAL DATA", otpSubtitle: "KOODHKA", otpVerifyingNumber: "Xaqiijinta lambarka",
    otpCodeIs: "Koodkaagu waa", otpEnterBelow: "Geli koodkan hoosta", otpEnterCode: "Koodka Xaqiijinta",
    ...
    // Settings page
    settingsTitle: "Dejinta", settingsTheme: "Muuqaalka", settingsThemeDesc: "Dooro sida SAHAL DATA u ekaan doonto",
    ...
    aboutBody: "SAHAL DATA wuxuu kuu ogolaanayaa inaad ka iibsato internet, wac, iyo SMS Hormuud, Somnet, Somtel, iyo Amtel hal app — leh abaalmarinta Macaash iibsi kasta.",
    ...
    inviteFriendsBody: "La wadaag SAHAL DATA saaxiibada iyo qoyska si ay u helaan adeeg wanaagsan.",
    inviteFriendsButton: "KU CASUUMO SAAXIIBADA",
    // Rate
    rateTitle: "Qiimee SAHAL DATA", rateSubtitle: "Sideed u aragtaa adeeggeena ilaa hadda?", notNow: "Hadda ha ahaan",
    // Delete account
...
const SAHAL_DATA_API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "";
const SAHAL_DATA_API_ENABLED = Boolean(SAHAL_DATA_API_BASE_URL);

let sahalDataAccessToken = null;

async function sahalDataApiRequest(path, { method = "GET", body, auth = false } = {}) {
  if (!SAHAL_DATA_API_ENABLED) throw new Error("SAHAL DATA API not configured (SAHAL_DATA_API_BASE_URL is empty)");
  const headers = { "Content-Type": "application/json" };
  if (auth && sahalDataAccessToken) headers.Authorization = `Bearer ${sahalDataAccessToken}`;
  const res = await fetch(`${SAHAL_DATA_API_BASE_URL}${path}`, {
    method,
    headers,
...
// One function per real endpoint this app needs — matches sahal-data-backend.zip's
// src/routes/*.js exactly (see sahal-data-backend-architecture.md §4 for the full
// contract). Every one of these is real, working code, not a stub; it's just
// unreachable until SAHAL_DATA_API_BASE_URL points at a deployed server.
const SahalDataApi = {
  requestOtp: (phone) => sahalDataApiRequest("/auth/otp/request", { method: "POST", body: { phone } }),
  verifyOtp: (phone, code) => sahalDataApiRequest("/auth/otp/verify", { method: "POST", body: { phone, code } }),
  getCompanies: () => sahalDataApiRequest("/companies"),
  getPackages: (companyId) => sahalDataApiRequest(`/companies/${companyId}/packages`),
  getBanners: () => sahalDataApiRequest("/banners"),
  createOrder: (companyId, packageId, receiverPhone, paymentMethod) =>
    sahalDataApiRequest("/orders", { method: "POST", auth: true, body: { companyId, packageId, receiverPhone, paymentMethod } }),
  getOrders: () => sahalDataApiRequest("/orders", { auth: true }),
  getOrder: (orderId) => sahalDataApiRequest(`/orders/${orderId}`, { auth: true }),
  getMacaashBalance: () => sahalDataApiRequest("/macaash/balance", { auth: true }),
  getMacaashRewards: () => sahalDataApiRequest("/macaash/rewards", { auth: true }),
  redeemMacaash: (rewardId) => sahalDataApiRequest("/macaash/redeem", { method: "POST", auth: true, body: { rewardId } }),
  getMacaashHistory: () => sahalDataApiRequest("/macaash/history", { auth: true }),
  submitFeedback: ({ category, message, deviceInfo }) =>
    sahalDataApiRequest("/feedback", { method: "POST", auth: true, body: { category, message, deviceInfo } }),
};
...
function OtpScreen({ phone, onVerified, onBack }) {
  ...
  // client-side. Demo mode (default, SAHAL_DATA_API_ENABLED = false) is completely
  // unaffected — this effect is a no-op unless a real API URL is configured.
  useEffect(() => {
    if (SAHAL_DATA_API_ENABLED) {
      SahalDataApi.requestOtp(phone).catch(() => setError(true));
    }
  }, [phone]);
  ...
  const verify = async () => {
    ...
    if (SAHAL_DATA_API_ENABLED) {
      setVerifying(true);
      try {
        const result = await SahalDataApi.verifyOtp(phone, code.join(""));
        sahalDataAccessToken = result.accessToken;
        onVerified();
      } catch {
        setError(true);
      } finally {
...
async function notifyOrderStatusChange(order) {
  ...
  try {
    ...
    await reg.showNotification("Sahal Data", {
...
function OrderDetailsScreen({ order, onBack, onOrderUpdated }) {
  ...
  const refresh = useCallback(
    async (silent) => {
      ...
      if (SAHAL_DATA_API_ENABLED) {
        try {
          const fresh = await SahalDataApi.getOrder(current.reference);
  ...
  // either — see sahal-data-backend.zip's README), but it delivers the same
  // practical result: the status updates without the user manually
  // refreshing the page.
  useEffect(() => {
    if (!SAHAL_DATA_API_ENABLED) return;
...
function FeedbackScreen({ onBack }) {
  ...
  const submit = async () => {
    ...
    setError("");
    try {
      await SahalDataApi.submitFeedback({
    ...
    content = (
      <PackagesScreen
        ...
        onBuy={async (pkg) => {
          if (SAHAL_DATA_API_ENABLED) {
            try {
              const realOrder = await SahalDataApi.createOrder(providerId, pkg.id);
    ...
    content = (
      <PaymentScreen
        ...
        onPaid={({ senderPhone, receiverPhone, paymentMethod }) => {
          const updated = {
            ...activeOrder,
            status: "in_progress", // payment confirmed; a real deployment moves this via the agent/admin verify flow — see sahal-data-backend.zip
            senderPhone,
            receiverPhone,
            paymentMethod,
