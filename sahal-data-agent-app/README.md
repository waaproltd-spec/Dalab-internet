# Sahal Data Agent App — Android module

A real Kotlin + Jetpack Compose Android app, wired to the deployed production
backend (`admin-backend-ts`, at `https://sahal-data-2.onrender.com/` —
see `ApiClient.kt`). It's a standalone Gradle project (own `gradlew`,
`settings.gradle.kts`) — open `sahal-data-agent-app/` directly in Android Studio, or build
...
to GitHub and the root-level `.github/workflows/sahal-data-agent-app-build-apk.yml`
...
download the `sahal-data-agent-debug-apk` artifact — that's a real, installable
...
- Visual polish is minimal — app icon, `ic_notification`, `strings.xml`, and
  theme colors exist (reusing the SAHAL DATA brand colors — indigo `#1D2E8C`,
...
`main` that touches `sahal-data-agent-app/`, alongside the existing debug build. Signing
...
```bash
keytool -genkeypair -v -keystore agent-release.jks -alias sahal-data-agent \
