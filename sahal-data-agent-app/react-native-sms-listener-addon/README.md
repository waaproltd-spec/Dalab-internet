# useSMSListener — Sahal Data Agent App (React Native)
...
The Agent App built earlier in this project (`sahal-data-agent-app.zip`) is
...
touch or replace `sahal-data-agent-app.zip`.
...
- **The API client matches the real, deployed backend exactly** —
  ...
  paths and body shapes as `sahal-data-backend.zip`, which has its own passing
...
```ts
import { useSMSListener } from "@sahaldata/agent-sms-listener/src/hooks/useSMSListener";
import { SessionManager } from "./your-existing-auth-module";

function AgentOrdersScreen() {
  const listener = useSMSListener({
    apiBaseUrl: "https://api.sahaldata.so", // once sahal-data-backend.zip is deployed
...
src/api/sahalDataAgentApi.ts      — typed client for the real backend routes
