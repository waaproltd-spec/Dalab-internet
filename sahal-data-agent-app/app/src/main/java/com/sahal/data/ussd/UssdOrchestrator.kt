...
import com.sahal.data.data.OrderStatus
import com.sahal.data.diagnostics.DiagnosticsLog
import com.sahal.data.network.ApiClient
import com.sahal.data.network.DialAttemptResultRequest
import com.sahal.data.network.DialAttemptStartRequest
import com.sahal.data.network.VerifyPaymentRequest
import com.sahal.data.queue.PendingActionQueue
import com.sahal.data.queue.RetryClassifier
...
class UssdOrchestrator(context: Context, private val maxAttempts: Int = 3) {
    ...
    private suspend fun processMatchedOrderLocked(orderId: String, smsLogId: String?): DialResult {
        ...
        // the backend's own USSD string generation (see sahal-data-backend.zip,
        ...
        for (attempt in 1..maxAttempts) {
            ...
            lastResult = when (val lookup = dialer.subscriptionIdForSlot(simSlot)) {
                ...
                SubscriptionLookupResult.PermissionMissing -> DialResult(
                    ...
                    "Required phone permissions aren't granted on this device — check Settings > Apps > Sahal Data Agent > Permissions.",
                )
