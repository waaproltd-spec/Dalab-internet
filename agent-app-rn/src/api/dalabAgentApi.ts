...
export function createDalabAgentApiClient(apiBaseUrl: string): DalabAgentApiClient {
  ...
  return {
    // POST /agent/sms-logs — server-side matches by amount + normalized
    // customer phone against pending orders across ALL providers (see
    // dalab-backend.zip src/routes/smsLogs.js) — deliberately cross-network:
    // the telecom that received the payment does not have to match the
    // telecom the customer ordered from. Returns matchedOrderId or null.
    // This is the real matching logic; the hook does not attempt its own.
