# Sahal Data Customer App — Android (Kotlin + Jetpack Compose)

The native Android counterpart to the [`customer-app`](../customer-app) React
web prototype, wired to the same production backend
(`admin-backend-ts`, at `https://sahal-data-2.onrender.com/` — see
`ApiClient.kt`). A standalone Gradle project (own `gradlew`,
`settings.gradle.kts`) — open `sahal-data-customer-app-android/` directly in Android
...
`sahal-data-customer-debug-apk` artifact. See "Release APK" below for a signed build.
...
uploads) on every push to `main` that touches `sahal-data-customer-app-android/`,
...
```bash
keytool -genkeypair -v -keystore customer-release.jks -alias sahal-data-customer \
