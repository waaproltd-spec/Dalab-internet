...
object ApiClient {
    ...
    // No backend is deployed anywhere reachable yet — this is a placeholder.
    // Production backend for this whole project: dalab-admin-backend-ts
    // (Node.js + Express + TypeScript + PostgreSQL) — NOT the earlier SQLite
    // backend, which is legacy/reference only. Once that TS backend is
    // deployed (Render/etc.), replace this with its real HTTPS URL. For
    // local testing against `npm run dev` on a dev machine from an Android
    // emulator, use "http://10.0.2.2:4000/" (the emulator's alias for the
    // host's localhost); from a physical device on the same LAN, use the
    // host machine's LAN IP. Response shapes are identical between the two
    // backends (verified field-by-field — see dalab-admin-backend-ts's
    // README), so nothing else in this app needs to change either way.
    private const val BASE_URL = "https://api.dalabinternet.so/"
