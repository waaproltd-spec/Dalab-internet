...
ALTER TABLE ussd_templates ADD COLUMN IF NOT EXISTS device_id TEXT REFERENCES agent_devices(id) ON DELETE SET NULL;
...
-- needs to act on it.
ALTER TABLE orders ADD COLUMN IF NOT EXISTS ussd_device_id TEXT REFERENCES agent_devices(id) ON DELETE SET NULL;
