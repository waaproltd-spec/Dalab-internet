...
const WALLET_COLUMNS = `id, name, provider_label, company_id, dial_prefix, logo_key, color_hex, enabled, sort_order, created_at, updated_at`;
 ...
 * wallets are returned. Joined to companies via company_id so each wallet
 * carries ITS OWN provider's payment number directly — this is the wallet's
 * own collection number, independent of whatever company's package the
 * customer is actually purchasing (paying via one telecom's wallet to buy a
 * different telecom's package is intentional and must never be conflated).
 */
paymentWalletsRouter.get("/payment-wallets", async (_req, res) => {
  const rows = await query(
    `SELECT pw.id, pw.name, pw.provider_label, pw.company_id, pw.dial_prefix, pw.logo_key, pw.color_hex,
            pw.enabled, pw.sort_order, pw.created_at, pw.updated_at,
            c.name AS provider_name, c.payment_number AS payment_number
     FROM payment_wallets pw
     LEFT JOIN companies c ON c.id = pw.company_id
     WHERE pw.enabled = true
     ORDER BY pw.sort_order, pw.name`
  );
...
paymentWalletsRouter.post("/admin/payment-wallets", requireAuth("super_admin"), async (req, res) => {
  const { id, name, providerLabel, companyId, dialPrefix, logoKey, colorHex, sortOrder } = req.body;
  ...
  // Optional-if-absent rather than mandatory for now — tolerates a stale
  // cached dashboard bundle mid-rollout that doesn't send it yet.
  if (companyId) {
  ...
  await query(
    `INSERT INTO payment_wallets (id, name, provider_label, company_id, dial_prefix, logo_key, color_hex, sort_order)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
    [id, name, providerLabel ?? null, companyId ?? null, dialPrefix, logoKey, colorHex ?? "#16A34A", sortOrder ?? 0]
  );
...
paymentWalletsRouter.put("/admin/payment-wallets/:id", requireAuth("super_admin"), async (req, res) => {
  ...
  const companyId = req.body.companyId !== undefined ? req.body.companyId : existing.company_id;
  if (companyId) {
    const company = await queryOne(`SELECT id FROM companies WHERE id=$1`, [companyId]);
    if (!company) return sendJson(res, 404, { error: "Company not found" });
  }
  ...
  await query(
    `UPDATE payment_wallets SET name=$1, provider_label=$2, company_id=$3, dial_prefix=$4, logo_key=$5, color_hex=$6, sort_order=$7, updated_at=now() WHERE id=$8`,
    [merged.name, merged.providerLabel, merged.companyId, merged.dialPrefix, merged.logoKey, merged.colorHex, merged.sortOrder, req.params.id]
  );
